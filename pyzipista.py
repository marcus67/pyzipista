...
class ZipHandler(object):
  ...
  def create_zip_file(self):
    ...
    zip_string_output = six.BytesIO()
    zip_file = zipfile.ZipFile(file=zip_string_output, mode="w", compression=zipfile.ZIP_DEFLATED)
    
    zip_file.comment = b'Generated by pyzipista.py (see https://github.com/marcus67/pyzipista).'
    zip_file.debug = 3
    ...
    base64_string = base64.b64encode(binary_zip_string, b'_&')
    unzip_template_file_as_string = file_access.load_into_string(UNZIP_TEMPLATE_FILE, "rb")    
    unzip_file_as_string = unzip_template_file_as_string.replace(BASE64_PLACEHOLDER, base64_string).replace(APP_NAME_PLACEHOLDER, bytearray(app_name,'utf-8')).replace(APP_URL_PLACEHOLDER, bytearray(app_url,'utf-8')).replace(MOD_TIMES_PLACEHOLDER, bytearray(mod_times_string,'utf-8'))
    
    zip_filename = self.get_zip_filename()
    with open(zip_filename, "wb") as unzip_file:
...
def load_config_file_and_check_zip_required(config_filename):
  ...
  try:
    ...
    logger.error("Error '%s' while checking file status" % str(e))  
    return None
...
def main():
  ...
  if len(sys.argv) == 2:  
    ...
    logger.error("pyzipista must called with filename of configuration file")
